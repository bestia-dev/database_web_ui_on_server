...
use crate::actix_mod::get_i32_from_query;
use crate::actix_mod::get_str_from_query;
use crate::actix_mod::return_response_no_cache;
use crate::deadpool_mod::get_client_from_pool;
...
pub async fn webpage_hits_list(db_pool: DbPool) -> ResultResponse {
    ...
    // region: 2. get data from database
    let client = get_client_from_pool(db_pool).await?;

    let query = "SELECT id, webpage, hit_count from webpage_hits;";
    let row_set = client
        .query(query, &[])
        .await
        .map_err(|err| AppError::QueryError {
            source_error: err,
            user_friendly: "".to_string(),
            developer_friendly: query.to_string(),
...
pub async fn webpage_hits_new(db_pool: DbPool) -> ResultResponse {
    ...
    // region: 1. get data from database
    let client = get_client_from_pool(db_pool).await?;
    let query = "SELECT id, webpage, hit_count from webpage_hits_new();";
    let row_set = client
        .query(query, &[])
        .await
        .map_err(|err| AppError::QueryError {
...
"#,
    ...
    // region: 3. mix presentation and data because of html
    if row_set.len() != 1 {
    ...
    let r = row_set
        .iter()
        .next()
        .expect("Already tested there is exactly one row.");
    let id: i32 = r.get(0);
    let webpage: String = r.get(1);
    let hit_count: i32 = r.get(2);
    let body = body
...
pub async fn webpage_hits_edit(
    ...
    // region: 0. parse params from web query
    let id = get_i32_from_query(&query, "id")?;
    // endregion

    // region: 1. send parameters to database
    let client = get_client_from_pool(db_pool).await?;
    let query = "SELECT id, webpage, hit_count from webpage_hits_read($1);";
    let row_set = client
        .query(query, &[&id])
        .await
        .map_err(|err| AppError::QueryError {
            source_error: err,
            user_friendly: "".to_string(),
            developer_friendly: format!("{} {}", query, id),
            source_line_column: format!("{}:{}:{}", file!(), line!(), column!()),
        })?;
    // endregion
...
pub async fn webpage_hits_insert(
    ...
    // region: 0. parse params from web query
    let id = get_i32_from_query(&query, "id")?;
    let webpage = get_str_from_query(&query, "webpage")?.to_string();
    let hit_count = get_i32_from_query(&query, "hit_count")?;
    // endregion
    ...
    let client = get_client_from_pool(db_pool).await?;
    let query = "SELECT id, webpage, hit_count from webpage_hits_insert($1, $2, $3);";
    let row_set = client
        .query(query, &[&id, &webpage, &hit_count])
        .await
        .map_err(|err| AppError::QueryError {
            source_error: err,
            user_friendly: "".to_string(),
            developer_friendly: format!("{} {} | {} | {}", query, id, &webpage, hit_count),
            source_line_column: format!("{}:{}:{}", file!(), line!(), column!()),
        })?;
    // endregion
...
pub async fn webpage_hits_update(
    ...
    // region: 0. parse parameters from web query
    let id = get_i32_from_query(&query, "id")?;
    let webpage = get_str_from_query(&query, "webpage")?.to_string();
    let hit_count = get_i32_from_query(&query, "hit_count")?;
    // endregion
    ...
    // and get data from database
    let client = get_client_from_pool(db_pool).await?;
    let query = "SELECT id, webpage, hit_count from webpage_hits_update($1, $2, $3);";
    let row_set = client
        .query(query, &[&id, &webpage, &hit_count])
        .await
        .map_err(|err| AppError::QueryError {
...
pub async fn webpage_hits_delete(
    ...
    let query = "SELECT webpage_hits_delete($1);";
    client
        .query(query, &[&id])
        .await
        .map_err(|err| AppError::QueryError {
